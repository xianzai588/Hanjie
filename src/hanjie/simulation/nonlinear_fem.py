...
def _assemble(nodes,elements,displacement,material,thermal_strain,old_states,active_elements):
    degrees=3*len(nodes)
...
def _contact_response(nodes,displacement,specification):
    ...
    degrees=3*len(nodes)
    force=np.zeros(degrees); stiffness=np.zeros((degrees,degrees))
    if not specification:
        return force,stiffness,None
    ...
    for local,node in enumerate(node_indices[active]):
        ...
        stiffness[np.ix_(dofs,dofs)]+=penalty*np.outer(gradient,gradient)
    report={
    ...
    for step_index,step in enumerate(steps):
        ...
        for iteration in range(max_iterations+1):
            ...
            try:
                ...
                increment=np.linalg.solve(effective_tangent[np.ix_(free,free)],residual[free])
            ...
            for line_search in range(12):
                ...
                candidate_internal,_,_,_,_=_assemble(nodes,elements,candidate.reshape((-1,3)),material_step,thermal,states,active)
