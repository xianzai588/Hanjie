...
def collect_current_status(root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "tolerance":{
            ...
            "product_worst_case_design_sum_mm":current_assessment(root)["precision"]["radial_sum_mm"],
...
def render_markdown(status: Dict[str, Any]) -> str:
    ...
    lines = ["# 自动生成的当前证据摘要","",status["generated_from"],"",
        ...
        f"终加工基准链七项分配为径向 {status['competition_design']['precision']['radial_sum_mm']:.4f} mm、直径 {status['competition_design']['precision']['design_diameter_budget_mm']:.4f} mm；加入目标测量扩展不确定度后为 {status['competition_design']['precision']['diameter_with_uncertainty_target_mm']:.4f} mm。A2候选最小几何径向加工余量 {status['competition_design']['machining_allowance']['candidates'][2]['geometric_min_radial_allowance_mm']:.3f} mm，既定压力与尺寸端点数字筛查已闭合；预加工座体和胀套状态已进入本次几何链，终镗包络、孔壁最薄处、胀套弹性重复性和实物制造能力仍待确认。", "",
        ...
        *[f"| {r['layout']} | {r['required_allowable_mpa']:.2f} | {r['net_heat_kj']:.2f} |" for r in status["competition_design"]["four_pass_comparison"]],
        f"固定送丝 {status['competition_design']['process']['fixed_feed_mm_s']:.3f} mm/s；等效焊脚 3.50～3.80 mm。疲劳尚未评定。",
        "## 历史研究入口", "ROUTE-B-DESIGN 的旧送丝与0.035 mm预算仅用于历史复现，见 studies/ROUTE-B-DESIGN；不得用作当前设计。",
