...
def build_story() -> list[Flowable]:
    ...
    story.extend([PageBreak(), P("9.3 视觉工程判定", "H2CN"), P("困难集揭示了“能返回结果”和“能用于定位”之间的差别：noise、illumination、perspective、occlusion、missing_edges 虽然返回率均为 100%，但 P95 超过设计门限，必须在现场链路中拒绝或转人工复核。", "BodyCN"), source_note("automation/vision/results/difficult-summary.json；姿态门限由 0.010 mm / 73.8 mm 换算为 0.0078°。"), *figure(CHARTS["vision"], "图 5  困难视觉条件的工程门结果；红色为 FAIL，不能以检测返回率替代。", 145 * mm), PageBreak()])
    ...
    story.extend([Spacer(1, 4 * mm), callout("评审关注：100% 是否“自己出题自己答”？——是当前注入模式上的规则回归结果，不是现场分布泛化证明；真实焊机数据将重标定阈值、误报率和延迟。", "amber"), Spacer(1, 6 * mm), P("11  洁净度是硬约束", "H1CN"), P("候选路线采用无焊渣工艺、内腔防护、焊前清洁、分段短焊、焊后目视和内窥检查。数字图、仿真和“无异常事件”都不能替代内腔实物检查；正式记录应建立一件一码、焊材批次、操作者、参数曲线、内窥照片和放行记录的关联。", "BodyCN"), callout("洁净度的最终证据不是算法准确率，而是焊后内窥/目视记录和可追溯放行表。", "red"), PageBreak()])
