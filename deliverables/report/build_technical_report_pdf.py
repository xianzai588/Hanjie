...
def on_page(canvas, doc) -> None:
    ...
    if page > 1:
        ...
        canvas.drawRightString(A4[0] - doc.rightMargin, A4[1] - 13 * mm, "数字工程参赛方案 | DESIGN-REVIEW")
    canvas.setStrokeColor(LINE)
    canvas.setLineWidth(0.5)
    canvas.line(doc.leftMargin, 13 * mm, A4[0] - doc.rightMargin, 13 * mm)
    canvas.setFont("Deng", 7)
    canvas.setFillColor(MUTED)
    canvas.drawString(doc.leftMargin, 8 * mm, "QT450-10 / Q235B | V2 | 非制造发布版")
...
def build_story() -> list[Flowable]:
    ...
    story.extend([
        ...
        P("TECHNICAL REPORT V2  |  FORMAL PDF / DESIGN BASELINE", "CoverSub"),
        Spacer(1, 8 * mm),
        ...
        table([
            ...
            ["V2 | 2026-09-02", "数字工程评审稿", "自动 TIG + 镍基填充\n六点短焊段 / S3", "柔顺方案仍为候选\n二维代理 FE 保留反例"],
    ...
    story.extend([Spacer(1, 4 * mm), P("线热输入", "H2CN"), callout("q_l = η U I / v = 0.55 × 12 × 75 / 1.5 = 330 J/mm  （公式 1）", "teal"), Spacer(1, 4 * mm), P("选择逻辑", "H2CN"), P("TIG 的价值在于热输入和路径可控、便于分段/跳焊和过程记录；镍基填充是降低异种材料界面风险的候选假设。ERNiFe-CI 对本题组合的直接有效性仍由真实接头小试、金相、硬度和裂纹/NDT结果裁决。", "BodyCN"), callout("一句话边界：这是“值得验证的工艺候选”，不是“已评定工艺”。", "amber"), PageBreak()])
    ...
    story.extend([Spacer(1, 5 * mm), P("夹具约束映射", "H2CN"), table([
        ...
        ["径向位置", "中心定位销轴线", "B"],
        ["切向/周向", "独立定位孔/时钟特征", "C"],
    ], [52 * mm, 66 * mm, 52 * mm], compact=True), Spacer(1, 4 * mm), callout("所有工程图状态均为 design-review；未完成三维关联、公差叠加、干涉检查和制造签审。", "amber"), PageBreak()])

    # 5 reduced order
    story.extend(section("降阶模型与位置度后处理", "5"))
    ...
    # 6 FE
    story.extend(section("二维热-结构代理交叉检查", "6"))
    ...
    # 8 vision and budget
    story.extend(section("自动化定位与工程误差预算", "8"))
    story.append(P("视觉数字样本用于验证算法链路，不代表真实工业相机/镜头/光源精度。工程门限采用 P95，避免少量大误差被 MAE 掩盖。", "BodyCN"))
    story.extend([P("8.1 Ø0.05 mm 的径向预算", "H2CN"), BudgetBar(), Spacer(1, 3 * mm)])
    story.append(table([
        ["来源", "径向预算 (mm)", "当前证据状态"],
    ...
    story.extend([Spacer(1, 5 * mm), P("13  物理验证接口", "H1CN"), P("样件和设备具备后，按“焊前基准测量 → 候选 WPS 焊接并记录 → 完全冷却并解除夹具 → CMM/校准替代测量 → 内孔分层拟合 → 外观、内窥、NDT、金相和硬度评价”的顺序执行。数字样本与实物数据使用不同 source_type，不互相覆盖。", "BodyCN"), PageBreak()])

    # 14/15/16 and referee answers
    story.extend(section("结论、提交状态与恶意评委速答", "14"))
    story.append(P("降阶模型和二维代理 FE 的排序不同，说明结果对结构表达、连接刚度和边界条件敏感。V2 的贡献不是提前宣称实物性能，而是把模型适用域、反例、自动化门限和下一步验证门完整串联。", "BodyCN"))
    story.extend([P("14.1 十个关键质疑的直接回答", "H2CN"), table([
        ["评委问题", "正文一句话回答"],
    ...
    story.extend([Spacer(1, 4 * mm), P("15  参赛提交 vs 制造放行", "H1CN"), table([
        ["状态", "V2 的结论"],
        ["参赛提交", "可作为数字工程设计方案提交；必须标明代理模型、困难视觉 FAIL 和未完成物理验证。"],
        ["制造放行", "必须补齐材料质保书、WPS/PQR/等效评定、真实焊接记录、焊后 CMM、金相/硬度/NDT、洁净度和设备标定。"],
    ], [35 * mm, 141 * mm])])
    story.extend([Spacer(1, 5 * mm), callout("当前候选路线：自动 TIG + 镍基填充、六点短焊段和 S3 作为继续验证的候选路线；同时保留连续座体/刚性夹具作为 FE 反例基准，最终结构由更高等级证据裁决。", "teal"), Spacer(1, 5 * mm), P("16  参考与数据源", "H1CN"), P("[1] 第一届辽宁省大学生材料焊接与铸造工艺设计大赛实施方案及固定命题附件。\n[2] docs/research/evidence-matrix.md：材料与焊接性证据矩阵。\n[3] simulation/、automation/、cad/：本项目脚本、输入配置、数字结果和工程表达图。\n[4] docs/validation/position-error-budget.md：位置误差预算及数字工程门限。", "BodySmall"), source_note("本 PDF 由 deliverables/report/build_technical_report_pdf.py 生成；结果来源和模型边界均在正文及对应归档文件中标注。")])
    return story
...
def main() -> int:
    ...
    doc = BaseDocTemplate(
        ...
        author="Hanjie digital engineering team",
    )
