...
终加工基准链七项分配为径向 0.0138 mm、直径 0.0276 mm；加入目标测量扩展不确定度后为 0.0296 mm。该设计分配不含焊接收缩；当前径向加工余量 0.20 mm 尚未闭合。
...
| 阶段 | 验收状态 | 允许用途 |
| --- | --- | --- |
| COMPETITION-R1 | 修订设计检查通过；实物性能未验证 | 四道沉积、修订胀套与连续薄裙、名义包络、七项终加工基准链0.0138 mm径向算术分配及互锁；焊接收缩/0.20 mm加工余量和CMM不确定度未闭合，不放行实物性能或正式热结构 |
...
| ROUTE-B-DESIGN | digital_recommendation_frozen_physical_release_not_available | 原沉积效率范围内的承载—净热输入—弧燃时间条件选型；附带的旧公差口径仅供历史复算，不进入当前位置度预算或0.20 mm加工余量判定 |
