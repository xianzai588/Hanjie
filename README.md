...
**当前参赛版：COMPETITION-R1（RC11，2026-09-23）。** 采用6P四道TIG、Ø1.6棒材固定送丝、圆柱胀套独立夹紧和连续薄裙底口回收。终加工基准链为0.0138 mm径向分配、Ø0.0296 mm含目标测量不确定度的设计分配；不含焊接收缩，当前0.20 mm径向加工余量未闭合。150℃仅为未放行候选，PWHT设三条比较分支；疲劳、实物精度、洁净和完整热—结构均未认证。固定题允许研究报告、设计图或研究实物，本作品采用报告与设计图形式；没有实物限制验证证据，不构成提交形式缺项。

- [当前说明书（页数以提交包 manifest 为准）](output/pdf/technical-report-v4.pdf)与[六页设计图](cad/generated/engineering-drawings/pdf/HJ-DRW-drawing-set.pdf)。
...
**2026-09-08 无实物条件选型已执行**：新增六候选108组承载—沉积截面—名义净热输入核算。固定送丝下显式计入原沉积效率区间，单道与四道分别计算焊脚、耗材和弧燃时间；按参考载荷/假设许用给出研究优先项，不发布完整工艺合格结论。当前报告已同步432例无稳健排序结论。执行入口为 `python studies/ROUTE-B-DESIGN/run.py`，研究边界见 [说明](studies/ROUTE-B-DESIGN/README.md)。当前无实物计划不要求采集实焊数据，历史热模型停止逐点调参。

**2026-09-23 COMPETITION-R1 RC10**：统一终加工基准链径向/直径口径，移除位置度链内的热残余分项；以TWI钢焊缝规则作材料范围受限的余量压力比较，并将0.20 mm状态登记为未闭合。修订焊材AWS分类边界、预热适用条件、疲劳敏感性、标准题名、证据等级和提交附件；修复视觉摘要与预算审计重复计入支点倾斜的问题，标记旧Route B公差反算为历史复现，并更新导航、文献核验和RC10哈希记录。

**2026-09-23 COMPETITION-R1 RC11**：把QT450-10批次证明设为预热、PWHT和WPS/PQR的阻断门；补全NiFe无PWHT、NiFe+PWHT、Ni-CI+PWHT三条比较路线。将52.17 MPa和1.150统一限定为静力喉部筛查值/比值，疲劳按焊趾、焊根/喉部和翼根/槽根独立评定并撤销未核实的FAT敏感性说法；将无实物影响限定为验证证据缺口，并重建含文献核验附件的提交包与RC11哈希。
...
**COMPETITION-R1 当前状态**（2026-09-23）：四道固定送丝守恒、夹具名义包络、七项终加工基准链算术和互锁逻辑已形成可复算设计；3.5 mm焊脚仍是设计目标，焊缝成形与熔合未评定。焊后位移、0.20 mm加工余量、测量不确定度、疲劳和洁净度均未闭合。各阶段允许用途见 [`project/stage-status.yaml`](project/stage-status.yaml)。

**V4.3 历史基线**：已完成七个真实三维实体的静力筛查；此后COMPETITION-R1把6P-FAIR_B四道TIG作为数字设计推荐。局部热模型只用于参数窗口和风险边界，温度门控、逐件装配预偏置和独立CMM仍需工程验证。
...
| 09-25 | 方案比较与结构优化（已完成/待评审） |
| 10-02 | 网格/参数敏感性/鲁棒性 |
| 10-08 | 自动化软件 MVP（已完成/待集成验证） |
| 10-13 | 技术说明书 V1（已完成，当前为 RC4） |
| 10-17 | 工程图、流程图、结果图 |
| 10-20 | 报名与内部技术审查截止 |
| 10-25 | 学校统一提交截止（官方） |
...
- [当前正式报告源 COMPETITION-R1（工艺设计说明书）](deliverables/report/technical-report-v4-unified.md)
