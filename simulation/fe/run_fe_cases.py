...
def export_bore_nodes(mesh: MeshTri, displacement: np.ndarray, config: dict[str, Any], path: Path) -> np.ndarray:
...
def write_case(case: dict[str, str], config: dict[str, Any], mesh_points: int) -> dict[str, Any]:
    ...
    bore_points = export_bore_nodes(mesh, displacement, config, case_dir / "bore-nodes.csv")
...
def plot_summary(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    axis.bar(labels, values, color=["#64748b", "#0f766e", "#b45309"])
    axis.axhline(0.05, color="#dc2626", linestyle="--", label="limit 0.05 mm")
    axis.set_ylabel("P_FE (mm)")
    axis.set_title("Three-case 2D FE position metric")
...
def main() -> int:
    ...
    cases = [
        ...
        {"case_id": "FE-002", "structure": "baseline", "fixture": "rigid", "sequence": "S2"},
    ...
    write_summary(rows, OUTPUT_ROOT / "results")
