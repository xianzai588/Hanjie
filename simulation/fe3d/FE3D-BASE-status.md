> 2026-09-06 更新：七个真实三维实体的线弹性静力筛查已在 `simulation/structural-v4/` 完成；本文件只追踪尚未完成的完整工件热—弹塑性与松夹分析。`src/hanjie/simulation/fe3d.py` 仍是历史经验代理。
...
**阶段**：G1 七实体静力筛查已完成；G2 完整工件热—弹塑性与松夹分析待开始
**FE 状态**：静力筛查结果为 `solver_result_unvalidated`；`fe3d.py` 为历史 surrogate prototype，不能替代 G2
**设备状态**：设备、材料和检测窗口待核实
**下一步**：使用 THERMAL-0.4R1 的审计结论和三候选（Continuous、6P、8P-FAIR_B）定义完整工件热—结构验证，不复用未收敛热场作精确位置度输入
